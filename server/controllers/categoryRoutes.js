...
const getCategories = asyncHandler(async (req, res) => {
    ...
    if (page < 1 || perPage < 1) {
        res.status(400);
        throw new Error("Page and perPage must be positive intergers");
    }
...
const createCategory = asyncHandler(async (req, res) => {
    ...
    const category = await Category.create({
        name,
        image: imageUrl || undefined, //Store image URL if provided, else underfined
        categoryType,
    });
    ...
    if (category) {
        ...
        if (image !== undefined) {
            if (image) {
                ...
                category.image = undefined; // clear image of empty string is provided
            }
        }
